...
static FILE *__output_handle;
static FILE *__input_handle;
#define printf(args...) fprintf(__output_handle, args)
...
static int str_to_code(char *str, char *delim)
{
    int code, count;
...
static int scrub_space(int code)
{
    return (code % 256== ' ') ? code / 256 : code;
}
...
add_links(char *c)
{
    ...
    */
    int i,j,nr;
    char *f, *g, *h;
    char *idtest[5]; /* url, mailto, www, ftp, manpage */

    out_length+=strlen(c);

    nr=0;
    idtest[0]=strstr(c+1,"://");
    idtest[1]=(*c=='<'&&c[strlen(c)-1]=='>')?strchr(c+1,'@'):0;
    idtest[2]=strstr(c,"www.");
    idtest[3]=strstr(c,"ftp.");
    idtest[4]=strchr(c+1,'(');
    for (i=0; i<5; i++) nr += (idtest[i]!=NULL);
    while (nr) {
	j=-1;
	for (i=0; i<5; i++)
...
scan_request(char *c) {
	...
	case V('f','t'):
		...
		int nr = scrub_space(str_to_code(c, 0));
		out_html(change_to_font(nr));
		c += strcspn(c, " \t\n");
	    }
	    c=skip_till_newline(c);
	    break;
	...
	case V4('M','2','S','S'):
		...
		wordlist[1] = h;
		h = NULL;
		(void) add_to_index(i, wordlist[2], &h);
		wordlist[2] = h;
		out_html("<a name=\"");
		out_html(label);
		out_html("\">&nbsp;</a>\n<");
...
// NOT THREAD SAFE, USES A STATIC VARIABLE FOR THE FILE HANDLE
signed char parse_manpage(const char *input_file, const char *output_file) {
	if (!(__output_handle = fopen(output_file, "w"))) return 0;
	FILE *f = fopen(input_file, "r");
#define error_page(args...) { error_page(args); if (f) fclose(f); return 0; }
	if (f == NULL)
...
#define XTRA 5
    ...
    if (buf)
	 free(buf);
	fclose(__output_handle);
	__output_handle = NULL;
	return 1;
}
...
error_page(char *s, char *t, ...) {
	 ...
	 if (__input_handle) {
		 fclose(__input_handle);
		 __input_handle = NULL;
	 }
}

#define error_page(args...) { error_page(args); abort(); }
char *
